...
import { schemaOwnerCreated } from "./form";
import { z } from "zod";
...
const schema = z.object({
  ...
  page: z.number(),
  total: z.number(),
  data: z.array(schemaOwnerCreated),
});

export type OwnerPaginated = z.output<typeof schema>;
...
export default function Owners() {
  ...
  const { data: owners, isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get<OwnerPaginated>(
          `/customers?page=${page}&limit=${limit}&search[type]=owner&search[last_name]=${qs}&search[first_name]=${qs}&search[cpf]=${qs}`,
        )
